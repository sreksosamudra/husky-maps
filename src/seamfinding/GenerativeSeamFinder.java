    ...
    private static class PixelGraph implements Graph<Node> {
        ...
        public class Pixel implements Node {
            ...
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                ...
                // If pixel is at the last column, connect it to sink node
                if (this.x == picture.width()-1) {
                    result.add(new Edge<>(this, sink,0));
                }
                // Else, connect to its right-up, right-mid, and right-down neighbors
                else {
                        // If pixel is at top-most row, connect with the adjacent pixels at the same row and one row below
                        if (this.y == 0) {
                            for (int i = this.y; i <= this.y + 1; i += 1) {
                                Pixel to = new Pixel(this.x + 1, i);
                                result.add(new Edge<>(this, to, f.apply(picture, this.x + 1, i)));
                            }
                        }
                        // If pixel is at bottom row, connect with the adjacent pixels at the same row and one row above
                        if (this.y == picture.height() - 1) {
                            for (int i = this.y-1; i <= this.y; i += 1) {
                                Pixel to = new Pixel(this.x + 1, i);
                                result.add(new Edge<>(this, to, f.apply(picture, this.x + 1, i)));
                            }
                        }
                        // Only if the neighbor is in the bounds of the picture, connect with the adjacent pixels
                        if (0 < this.y && this.y < picture.height()-1) {
                            for (int z = this.y - 1; z <= this.y + 1; z += 1) {
                                Pixel to = new Pixel(this.x + 1, z);
                                result.add(new Edge<>(this, to, f.apply(picture, this.x + 1, z)));
                            }
                        }
                    }
